...
<div class="formbold-main-wrapper">
    ...
    <div class="formbold-form-wrapper">
        <form action="{{ route('appointmentone.store') }}" method="POST">
            @csrf
...
{{ session( 'failure' ) }}
            ...
            <div class="formbold-mb-5">
                <label for="phone" class="formbold-form-label"> Name Doctor </label>
                <select  name="did"  class="formbold-form-input" >
                    <option selected>--Choose the Doctor--</option>

                @foreach ( $appointmentone as $id=>$name )

                @if ($name->specialty=='Public Healthy')
